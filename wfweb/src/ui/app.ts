...
export class App {
  ...
  /** Valida o estado atual ao vivo, atualiza o painel de alertas, os badges das camadas e o estado do
   *  botão Export. Barato (O(camadas)); só toca o DOM quando a lista de problemas muda de fato. */
  private validate(): void {
    if (!this.dial) return;
    this.issues = validateDial(this.dial);
    const sig = this.issues.map((x) => `${x.level}:${x.layer ?? -1}:${x.msg}`).join("|");
    if (sig === this.lastIssueSig) return; // nada mudou → não re-desenha o painel/lista
    this.lastIssueSig = sig;
    const errs = errorsOf(this.issues);
    // botão Export: desabilitado + vermelho quando há erro duro (não instala).
    const exp = $("btnExport") as HTMLButtonElement;
    exp.disabled = errs.length > 0;
    exp.classList.toggle("danger", errs.length > 0);
    // painel de validação
    const panel = $("validation");
    if (this.issues.length === 0) {
      panel.hidden = false;
      panel.className = "validation ok";
      panel.innerHTML = `<span class="vhead">✓ Tudo instala</span><span class="vsub">same-footprint, edições in-place</span>`;
    } else {
      panel.hidden = false;
      panel.className = `validation ${errs.length ? "err" : "warn"}`;
      const head = errs.length ? `⚠ ${errs.length} problema(s) — Export bloqueado` : `${this.issues.length} aviso(s)`;
      const items = this.issues
        .map((x) => `<li class="${x.level}" ${x.layer !== undefined ? `data-layer="${x.layer}"` : ""}>${x.msg}</li>`)
        .join("");
      panel.innerHTML = `<span class="vhead">${head}</span><ul class="vlist">${items}</ul>`;
      panel.querySelectorAll<HTMLElement>("li[data-layer]").forEach((li) => {
        li.addEventListener("click", () => { this.selected = Number(li.dataset.layer); this.buildLayerList(); this.buildInspector(); this.render(); });
      });
    }
    this.buildLayerList(); // re-desenha badges ⚠ por camada
  }

  /** Índices das camadas com pelo menos um problema (p/ badge na lista). */
  private layerIssueLevel(i: number): "" | "warn" | "err" {
    let lvl: "" | "warn" | "err" = "";
    for (const x of this.issues) if (x.layer === i) { if (x.level === "error") return "err"; lvl = "warn"; }
    return lvl;
  }
  ...
  private buildLayerList(): void {
    ...
    d.layers.forEach((l, i) => {
      ...
      const lvl = this.layerIssueLevel(i);
      const badge = lvl ? `<span class="lbadge ${lvl}" title="Tem problema — veja o painel de validação">⚠</span>` : "";
      li.innerHTML = `<span>${iconFor(l.kind)}</span><span class="lname">${l.name}</span>${badge}<span class="kind">${l.cf ? "cf" + l.cf : ""} ${l.w}×${l.h}</span>` +
  ...
  private buildInspector(): void {
    ...
    // Barra de ORÇAMENTO do slot (só p/ camadas com asset próprio de imagem única). "usado" = payload
    // novo (ou o len original). Vermelho quando estoura o same-footprint.
    const hasSlot = l.assetOff > 0 && l.assetLen > 0 && !l.multiFrame && !l.frameSheet;
    let budgetRow = "";
    if (hasSlot) {
      const used = usedBytes(l), cap = l.assetLen, pct = Math.min(100, Math.round((used / cap) * 100));
      const over = used > cap;
      budgetRow = `<div class="field full budget ${over ? "over" : ""}">
        <div class="budget-head"><span>Orçamento do slot</span><span class="budget-num">${used} / ${cap} B (${over ? ">100" : pct}%)</span></div>
        <div class="budget-bar"><div class="budget-fill" style="width:${pct}%"></div></div>
        <div class="budget-sub">${over ? "não cabe — reduza/simplifique a arte" : `sobram ${cap - used} B p/ re-skin`}</div>
      </div>`;
    }
  ...
  /** Redimensiona um sprite raster: reescala a imagem atual p/ nw×nh e encaixa no slot via
   *  fitImageToSlot (degrada só o necessário; se não couber em nível nenhum, REVERTE p/ o tamanho
   *  anterior e não aplica). Same-footprint garantido. */
  private async resizeLayer(l: Layer, nw: number, nh: number): Promise<void> {
    if (!this.dial || l.cf === 1) return;
    const oldW = l.w, oldH = l.h, oldResized = l.resized, oldPayload = l.newPayload;
    try {
      const payload = l.newPayload ?? this.dial.raw.subarray(l.assetOff + 8, l.assetOff + 8 + l.assetLen);
      const cur = decodeAssetToRgba(payload, l.cf, oldW, oldH); // RGBA nas dimensões atuais
      const scaled = rescaleRgba(cur, oldW, oldH, nw, nh);
      l.w = nw; l.h = nh; l.resized = true;
      const fit = await this.fitImageToSlot(l, scaled); // encaixa no slot (ou null)
      if (!fit) {
        l.w = oldW; l.h = oldH; l.resized = oldResized; l.newPayload = oldPayload; // reverte
        this.buildInspector(); this.render(); this.refreshJson();
        this.status(`Resize ${nw}×${nh} descartado — não cabe no slot (${l.assetLen}B) nem após comprimir. Tamanho anterior mantido.`, "err");
        return;
      }
      this.buildInspector();
      this.render();
      ...
      this.status(`Resized ${nw}×${nh}${extra} (${fit.size}B ≤ ${l.assetLen}B). ✓ Same-footprint (resize in-place; teste no relógio).`, "ok");
    } catch (err) {
      l.w = oldW; l.h = oldH; l.resized = oldResized; l.newPayload = oldPayload;
  ...
  private onExport(): void {
    if (!this.dial) return;
    // Bloqueio ao vivo: se há erro duro no estado, nem tenta exportar.
    const liveErrs = errorsOf(this.issues);
    if (liveErrs.length) {
      this.status(`Export bloqueado — ${liveErrs[0].msg}${liveErrs.length > 1 ? ` (+${liveErrs.length - 1})` : ""}`, "err");
      return;
    }
    try {
      const bytes = this.encodeBytes();
      // Rede de segurança FINAL: re-parseia o resultado; recusa o download se algo ficou inválido.
      const binErrs = errorsOf(validateBin(bytes, this.dial));
      if (binErrs.length) {
        this.status(`Export abortado (bin inválido): ${binErrs.map((e) => e.msg).join(" · ")}`, "err");
        return;
      }
